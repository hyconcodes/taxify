...
<x-layouts::app :title="__('Dashboard')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        <flux:heading size="xl" class="mb-2">{{ __('Taxify Dashboard') }}</flux:heading>

        <div class="grid auto-rows-min gap-4 md:grid-cols-3">
            <flux:card class="flex flex-col items-center justify-center p-6">
                <flux:heading size="3xl">{{ $stats['vehicles'] }}</flux:heading>
                <flux:text class="mt-1">{{ __('Registered Vehicles') }}</flux:text>
            </flux:card>

            <flux:card class="flex flex-col items-center justify-center p-6">
                <flux:heading size="3xl">{{ $stats['captures'] }}</flux:heading>
                <flux:text class="mt-1">{{ __('Plate Captures') }}</flux:text>
            </flux:card>

            <flux:card class="flex flex-col items-center justify-center p-6">
                <flux:heading size="3xl" class="{{ $stats['alerts'] > 0 ? 'text-red-600 dark:text-red-400' : '' }}">
                    {{ $stats['alerts'] }}
                </flux:heading>
